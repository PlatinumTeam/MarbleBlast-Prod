...
$query = $pdb->prepare("SELECT *, LOWER(`username`) AS `user` FROM eventcandy");
$query->execute();
...
while (($row = $query->fetch(PDO::FETCH_ASSOC)) !== false) {
	...
	echo("Import candy on {$mission->name} for {$row["username"]}\n");
    ...
    if ($has->fetchColumn(0) > 0) {
        echo("User {$username} already has candy on {$mission->name}\n");
        continue;
    }
