...
$query = $pdb->prepare("SELECT *, LOWER(`username`) AS `user` FROM `serverscores`");
$query->execute();
...
//	echo("Found score at     {$time} on {$basename} score of {$score} for {$username}\n");
	...
	$gamemode = strtolower($mission->gameInfo["force_gamemode"] === "" ? $mission->gamemode : $mission->gameInfo["force_gamemode"]);
	...
	$gems10 = 0;
	//Uh
	if (strtolower($gamemode) === "hunt") {
		$gems10 = ($score - $gems1 - ($gems2 * 2) - ($gems5 * 5)) / 10;
//		if ($gems10 > 0) {
//			echo("Derp found $gems10 platinum gems\n");
//		}
	}

	$scoreType = "score";
	if ($gamemode === "coop") $scoreType = "time";
	if ($score > 2000) $scoreType = "time"; //Safety
	if ($score < 500) $scoreType = "score"; //Safety
//	echo("Mission: {$mission->basename}/{$row["level"]} Mode: $gamemode Score: $score Type: $scoreType\n");

	if ($scoreType === "score" && $gamemode === "coop") {
//		echo("Score on coop... fuck this\n");
		continue;
	}
