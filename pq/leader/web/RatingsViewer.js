...
var ratingColumns = [
	...
	[
		[
			...
			{"field": "disabled", "type": "text"}
		],
		[
			{"field": "revert", "type": "button"}
		],
		[
			{"field": "update", "type": "button"}
		]
	]
];
				...
				case "button":
					...
					if (field.field === "revert") {
						...
						button.click(function() {
							...
							//Tell the database!
							submitRatingInfo(extendedInfo);

							//Change the button so you know it works
					...
					var fieldDiv = $("<div>")
						...
						.append($("<input>")
							...
							.change(function() {
								...
								var oldVal = $(this).val();
								var newVal = eval(oldVal);

								//IDK if this calls change() but i'm not risking it
								changing = true;
								$(this).val(newVal);
								changing = false;
...
function submitRatingInfo(extendedInfo) {
	$.ajax({
		method: "POST",
		url: config.base + "/web/UpdateRatingInfo.php",
		data: {"info" : extendedInfo}
	}).done(function(data) {
		updateRatingInfo(extendedInfo);
		getRatingChangeLog(extendedInfo.missionInfo.id);
		getAllRatingChangeLogs();
	});
}

function getRatingChangeLog(missionId) {
	$.ajax({
		method: "POST",
		url: config.base + "/web/GetRatingChangeLog.php",
		dataType: "json",
		data: {"missionId" : missionId}
	}).done(function(data) {
		var changeLogDiv = $("#ratingChangeLog");
		changeLogDiv.empty();
		...
		data.forEach(function(change) {
			var dateDiv = $("<div>")
				...
				.append($("<button>")
					...
					.click(function() {
						changesDiv.empty();
						var changes = JSON.parse(change.changes);
						Object.keys(changes).forEach(function(key) {
							var oldValue = changes[key].old;
							var newValue = changes[key].new;

							var changeDiv = $("<div>")
								.text("Changed " + columnNames[key] + " from " + oldValue + " to " + newValue);
							changesDiv.append(changeDiv);
						});
					}));
			listDiv.append(dateDiv);
		});

		changeLogDiv.append(listDiv);
		changeLogDiv.append(changesDiv);
	});
}

function getAllRatingChangeLogs() {
	$.ajax({
		method: "POST",
		url: config.base + "/web/GetAllRatingChangeLogs.php",
		dataType: "json"
	}).done(function(data) {
		var changeLogDiv = $("#allChangeLogs");
		changeLogDiv.empty();
		...
		data.forEach(function(info) {
			var change = info.changeLog;
			var missionInfo = info.missionInfo;
				...
				.append($("<button>")
					...
					.click(function() {
						...
						changesDiv.append($("<div>")
							.text("Changes on level " + missionInfo.name + ":"));
						changesDiv.append($("<button>")
							.text("Go to Level")
							.addClass("btn")
							.addClass("btn-primary")
							.click(function() {
								selectMission(missionInfo.game_id, missionInfo.difficulty_id, missionInfo.id);
								$("#allChangeLogsModal").modal("hide");
							}));
...
function getGlobalTopScores(extendedInfo) {
	$.ajax({
		method: "POST",
		url: config.base + "/web/GetAllMissionScores.php",
		dataType: "json",
		data: {"missionId": extendedInfo.missionInfo.id, "modifiers": currentModifier}
	}).done(function(data) {
		var $container = $("#topScoresTable");
		$container.empty();

		var headerTemplate = Handlebars.compile($("#topScoresHeaderTemplate").html());
		var adderTemplate = Handlebars.compile($("#topScoresAdderTemplate").html());
		var rowTemplate = Handlebars.compile($("#topScoresTemplate").html());
		...
		function createAdder() {
			//Adder for inserting new times into the db
			var $adder = $(adderTemplate({}));

			var adderScore = $adder.find("input[data-attr=score]");
			var adderRating = $adder.find("#topScoresAdderRating");
			var adderScoreType = $adder.find("#topScoresAdderScoreType");

			//Gross! But finds out if this should be a score or a time
			var mode = extendedInfo.missionInfo.gamemode;
			var scoreType = (mode === "hunt" || (mode === "gemmadness" && !(currentModifier & modifierFlags.gemMadnessAll))) ? "Score" : "Time";
			adderScoreType.text(scoreType);

			$adder.find("input.form-control").keyup(function() {
				//Calculate rating
				var rating = getRating(adderScore.val(), extendedInfo, currentModifier);
				adderRating.text(rating);
			});
			$adder.find("button.btn.add").click(function() {
				var info = {
					modifiers: currentModifier,
					missionId: extendedInfo.missionInfo.id,
					score_type: scoreType.toLowerCase()
				};

				var $fields = $adder.find("input");
				for (var i = 0; i < $fields.length; i ++) {
					var $field = $($fields[i]);
					info[$field.attr("data-attr")] = $field.val();
				}
				...
				$.ajax({
					...
					//Reset the view
					$adder.remove();
					var $newAdder = createAdder();
					$newAdder.insertAfter($header);
					...
					//Add the row
					var $row = $(rowTemplate(info));
					$row.insertAfter($newAdder);
					...
					$row.find("button.delete").click(function() {
						...
						$.ajax({
							method: "POST",
							url: config.base + "/web/RemoveScore.php",
							dataType: "json",
							data: {scoreId: id}
						}).done(function(data) {
							if (data.error === "none") {
		...
		$container.find("button.delete").click(function() {
			var $this = $(this);
			var id = $this.attr("data-id");
			$.ajax({
				method: "POST",
		...
		var modifierDiv = $("<div>")
			...
			.append($("<input>")
				...
				.change(function() {
					...
					generateRatingPlot(extendedInfo, currentModifier);
					getGlobalTopScores(extendedInfo);
				})
			)
			.append(" ")
			.append($("<label>")
...
function onSelectMission(gameId, difficultyId, missionId) {
	...
	$.ajax({
		method: "POST",
		url: config.base + "/web/GetRatingInfo.php",
		dataType: "json",
		data: {
			missionId: missionId
		}
		...
		$("#rrecDownloadReplay")
			...
			.append(
				$("<li/>").append(
					$("<a/>").attr("href", "https://marbleblast.com/pq/leader/data/Replay/" + missionId + ".rrec.zip").text("Top rrec #1")
				)
			).append(
				$("<li/>").append(
					$("<a/>").attr("href", "https://marbleblast.com/pq/leader/data/Replay/" + missionId + ".rrec.1.zip").text("Top rrec #2")
				)
			).append(
				$("<li/>").append(
					$("<a/>").attr("href", "https://marbleblast.com/pq/leader/data/Replay/" + missionId + ".rrec.2.zip").text("Top rrec #3")
				)
			);
		$("#rrecDownloadEggReplay")
			.empty()
			.append(
				$("<li/>").append(
					$("<a/>").attr("href", "https://marbleblast.com/pq/leader/data/EggReplay/" + missionId + ".rrec.zip").text("Top egg rrec #1")
				)
			).append(
				$("<li/>").append(
					$("<a/>").attr("href", "https://marbleblast.com/pq/leader/data/EggReplay/" + missionId + ".rrec.1.zip").text("Top egg rrec #2")
				)
			).append(
				$("<li/>").append(
					$("<a/>").attr("href", "https://marbleblast.com/pq/leader/data/EggReplay/" + missionId + ".rrec.2.zip").text("Top egg rrec #3")
				)
			);
	});
...
function getMissionList() {
	...
	$.ajax({
		method: "POST",
		url: config.base + "/web/GetMissionListNoDisabled.php",
		dataType: "json"
	}).done(function(data) {
		var selectGame = $("#selectGame");
		var selectDifficulty = $("#selectDifficulty");
