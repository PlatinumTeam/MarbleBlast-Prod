...
if (array_key_exists("server", $_POST)) {
	...
	if ($result->rowCount()) {
		...
		if ($pid <= 0) {
			...
			$ps = explode("\n", $ps);
			//Grep list:
			$list = json_decode($array["greplist"], true);
			foreach ($list as $item) {
				//! at the start means inverse
				$inv = (bool)(substr($item, 0, 1) === "!");
				//Strip any !
				$item = str_replace("!", "", $item);
				//Find all processes
				foreach ($ps as $process) {
					//If they don't have it, or do with inverse
					if ((strpos($process, $item) === false) === ($inv ? false : true)) {
						$idx = array_search($process, $ps);
						unset($ps[$idx]);
					}
				}
			}
			...
			//Now we have $ps
			//sed -r 's/^\s*([0-9]+).*+/\1/g'
			if (preg_match('/^\s*([0-9]+).*+/', $ps, $matches)) {
				//Should have an identifier now
				//$send["ps"] = $ps;
...
<div class="container-fluid">
	<div class="row-fluid">
		...
		<div class="span9 well">
			<h1 class="text-center">Dedicated Servers</h1>
			<br>
			<br>
			<div class="row-fluid">
